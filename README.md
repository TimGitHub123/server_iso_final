** Учебный стенд для обучения инструментам DE **
Что входит:
01. Airflow;
02. S3 MiniO;
03. Spark;
04. Trino;
05. Iceberg;
06. Superset;
07. ClickHouse (2 шарды 2 реплики, в кластере 'c2sh2rep') - за помощь в настройке шардированного Клика благодарность Дмитрию Дементьеву https://t.me/dementev_tech
08.
09.

** Обращаю внимание, что для Airflow и Superset создается по одному контейнеру PosgreSQL для каждого
** Можно попробовать перед созданием контейнера как то создавать БД Superset и Airflow в одном экземпляре PosgreSQL
*** Также обращаю внимание, что у Airflow и Redis включены ограничения памяти, чтобы их снять удали полностью раздел deploy со всеми подразделами

## Минимальные Системные требования **
- Docker и Docker Compose
- ОЗУ 8 Gb (можно дать сразу 10 Gb, Spark пошустрее будет)
- CPU 4 ядра (тут тоже можно поиграться)
- HDD 20 Gb
- Свободные порты: 2182, 8002-8005, 8080-8089, 8124, 8181, 8888, 9001, 9123-9126

### Состав ClickHouse

- **4× ClickHouse Server** (версия 25.1) — основные узлы кластера
- **1× ZooKeeper** (3.9.3) — координатор для репликации
- **1× HAProxy** (3.1.7) — балансировщик нагрузки
- **1 тип кластерной конфигурации**: 2×2 (шарды×реплики)

Заходим через PUTTY на ПК с Ubuntu
по умолчанию мы будем в папке пользователя /home/ubuntu

# Создаем папку для нашего проекта, к примеру **springam** (имя критично, к нему привязан контейнер Airflow
```bash
mkdir springam
```
** переходим в нее командой только для просмотра файлов:
```bash
cd springam
```

** Через WinSCP грузим папки и файлы из архива 04_airflow_trino_superset_clickhhouse.zip внутрь папки /home/ubuntu/springam


#запускаем песочницу! (иногда может быть прерывание процесса с сообщением connection timeout - просто запускаем еще раз команду sudo docker compose up -d )
sudo docker compose up -d

### Проверка работоспособности

** для статусов контейнеров
```bash
docker compose ps
```

** проверка работы кластера ClickHouse
```bash
# Через HAProxy (рекомендуемый способ)
curl "http://default:123456@localhost:8124" -d "SELECT version()"

# Или через прямое подключение к узлу
curl "http://default:123456@localhost:9123" -d "SELECT version()"
```

## 🔧 Конфигурация кластера

### Доступные кластерные топологии

| Кластер    | Шарды | Реплики |           Назначение     |
|------------|-------|---------|--------------------------|
| `c2sh2rep` |   2   |    2    | Сбалансированное решение |
|------------|-------|---------|--------------------------|

### Порты и доступ

| Сервис    | HTTP | Native | Назначение                     |
|-----------|------|--------|--------------------------------|
| HAProxy   | 8124 |  9001  | Балансировка (основной доступ) |
| click1    | 9123 |  8002  | Прямой доступ для отладки      |
| click2    | 9124 |  8003  | Прямой доступ для отладки      |
| click3    | 9125 |  8004  | Прямой доступ для отладки      |
| click4    | 9126 |  8005  | Прямой доступ для отладки      |
| ZooKeeper |   -  |  2182  | Координация кластера           |
|-----------|------|--------|--------------------------------|

### Аутентификация

- **Пользователь**: `default`
- **Пароль**: 		`1q2w3e4r`

### Проверка кластерной конфигурации
```sql
-- Список доступных кластеров
SHOW CLUSTERS;
```

IP для инструментов:

| № |		  Название 	    |		      	Адрес		    	|     login	    |      pass     |       База      |
|:---:|:-------------------|:-------------------------|:---------------|:---------------|:-----------------|
| 1 |	Airflow			      |	http://11.11.1.22:8081/ | airflow       | airflow       |                 |
| 2 |	S3 MiniO		      | http://11.11.1.22:9001/	| minioadmin    | minioadmin    |                 |
| 3 |	Spark UI		      |	http://11.11.1.22:8089/	|        		    |   		        |                 |
| 4 |	superset		      |	http://11.11.1.22:8088/	| superset      | superset         |                 |
| 5 |	trino   		      |	http://11.11.1.22:8080/ | admin         |   	          |                 |
| 6 |	spark-iceberg     |	http://11.11.1.22:8888/ |    		        |  	 	          |                 |
| 7 |	rest		     	    |	http://11.11.1.22:8181/	|        		    |   	    	    |                 |
|	  | postgres airflow	|	  11.11.1.22 port 5433  |  airflow      |  airflow      | airflow         |
|	  | postgres_superset |	  11.11.1.22 port 5434  |  superset     |  superset     | superset        |
|	  | postgres-iceberg	|	  11.11.1.22 port 5435	|  iceberg      |  iceberg      | iceberg_catalog |
|	  | bi_postgres      	|	  11.11.1.22 port 5436  |  bi_postgres  |  bi_postgres  | bi_bd           |

** Для перезапуска инфраструктуры, если что-то пошло не так:**

```bash
docker-compose down && docker-compose build && docker-compose up -d  
```


Команды для Docker
# покажет использование памяти каждым контейнером.
sudo docker stats

# Команда с именем, статусом и портами
# Флаг -a показывает все контейнеры (включая остановленные)
docker ps -a --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"


# Удалить все проекты находясь в папке с файлом docker-compose
sudo docker compose down --rmi all --volumes --remove-orphans

# С очисткой volumes (осторожно!)
sudo docker system prune -a --volumes


# Неиспользуемые сети
sudo docker network prune

# Что будет удалено (просмотр без удаления)
sudo docker system df
docker system df -v

# Показать неиспользуемые ресурсы
docker container ls -a --filter status=exited
docker images -f dangling=true
docker volume ls -f dangling=true
docker network ls --filter type=custom

# Удаление всех хвостов (!!! Аккуратно, удаление без подтверждений !!!)
sudo docker compose down --rmi all --volumes --remove-orphans
sudo docker system prune -a --volumes -f
sudo docker network prune -f

# для superset
sudo docker compose --profile init up -d

# 1. Создать группу docker (если не существует)
sudo groupadd docker

# 2. Добавить текущего пользователя в группу docker
sudo usermod -aG docker $USER

# 3. Применить изменения группы (БЕЗ перезагрузки)

newgrp docker
